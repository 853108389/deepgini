...
def exp(model_name, coverage, use_adv, std=0.0, k=1, k_bins=1000):  #
    ...
    if coverage == 'kmnc':
        pass
        df.to_csv('./all_output/output_mnist/{}/{}_{}_k_bins_{}.csv'.format(model_name, dataset, coverage, k_bins))  #
    elif coverage == 'nbc':
        pass
        df.to_csv('./all_output/output_mnist/{}/{}_{}_std_{}.csv'.format(model_name, dataset, coverage, std))
    elif coverage == 'snac':
        pass
        df.to_csv('./all_output/output_mnist/{}/{}_{}_std_{}.csv'.format(model_name, dataset, coverage, std))
    elif coverage == 'tknc':
        pass
        df.to_csv('./all_output/output_mnist/{}/{}_{}_k_{}.csv'.format(model_name, dataset, coverage, k))


def exp_nac(model_name, use_adv, t):  #
    input, layers, test, train, pred_test, true_test, pred_test_prob = gen_data(model_name,
                                                                                use_adv=use_adv, deepxplore=True)
    rank_lst2 = None
    # 可以贪心排
    # 可以单个样本比较排
    # 0 0.5 1
    ...
    df.to_csv('./all_output/output_mnist/{}/{}_nac_t_{}.csv'.format(model_name, dataset, t))  #
...
def exp_deep_metric(model_name, use_adv):
    ...
    if use_adv:
        ...
        dataset = 'mnist'
    df.to_csv('./all_output/output_mnist/{}/{}_deep_metric.csv'.format(model_name, dataset))
